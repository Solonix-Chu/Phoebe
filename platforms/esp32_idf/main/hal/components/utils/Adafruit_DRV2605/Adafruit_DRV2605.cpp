...
*/
/**************************************************************************/
bool Adafruit_DRV2605::init(i2c_port_t i2cPort)
{
    //   if (!i2c_dev->begin())
    //     return false;
    _i2c_port = i2cPort;
...
uint8_t Adafruit_DRV2605::readRegister8(uint8_t reg)
{
    ...
    uint8_t buffer = 0;
    i2c_master_write_read_device(_i2c_port, DRV2605_ADDR, &reg, 1, &buffer, 1, portMAX_DELAY);
    return buffer;
}
...
void Adafruit_DRV2605::writeRegister8(uint8_t reg, uint8_t val)
{
    ...
    uint8_t buffer[2] = {reg, val};
    i2c_master_write_to_device(_i2c_port, DRV2605_ADDR, buffer, 2, portMAX_DELAY);
}
