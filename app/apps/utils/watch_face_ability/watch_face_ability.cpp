...
WatchFaceAbility::WatchFaceAbility()
{
    ...
    _data.duktape_ctx = duk_create_heap_default();
    duk_console_init(_data.duktape_ctx, DUK_CONSOLE_STDOUT_ONLY);
}

WatchFaceAbility::~WatchFaceAbility()
{
    duk_destroy_heap(_data.duktape_ctx);
}

void WatchFaceAbility::pushScript(const char* scriptContent)
{
    duk_eval_string(_data.duktape_ctx, scriptContent);
}

void WatchFaceAbility::onCreate()
{
    mclog::tagInfo(_tag, "on create");
    duk_helper::call_script_function(_data.duktape_ctx, _wf_callback_on_create);
}

void WatchFaceAbility::onResume()
{
    mclog::tagInfo(_tag, "on resume");
    duk_helper::call_script_function(_data.duktape_ctx, _wf_callback_on_resume);
}

void WatchFaceAbility::onRunning()
{
    // Call on tick every 1s
    if (HAL::SysCtrl().millis() - _data.tick_time_count > 1000) {
        duk_helper::call_script_function(_data.duktape_ctx, _wf_callback_on_tick);
        _data.tick_time_count = HAL::SysCtrl().millis();
    }
}
...
void WatchFaceAbility::onPause()
{
    ...
    duk_helper::call_script_function(_data.duktape_ctx, _wf_callback_on_pause);
}
